...
select BIDDER_NAME, bid_status, round(percent_rank() over (order by bidder_name),2) as percent_won
from ipl_bidder_details i join ipl_bidding_details n
on i.bidder_id = n.BIDDER_ID 
...
change bid_team TEAM_ID int(2);

select team_name, count(bid_status)
from ipl_team i right join ipl_bidding_details n
on i.TEAM_ID= n.TEAM_ID
where BID_STATUS ='bid'
group by TEAM_NAME
order by count(BID_STATUS) asc ;


#3 In a given stadium, what is the percentage of wins by a team which had won the toss?

select stadium_id , stadium_name ,
(select count(*) from ipl_match i inner join ipl_match_schedule m on i.match_id = m.match_id
where m.stadium_id = s.stadium_id 
and 
(toss_winner = match_winner)) /
(select count(*) from ipl_match_schedule m where m.stadium_id = s.stadium_id) * 100 
as 'Wins by teams who won the toss (%)'
from ipl_stadium s;


#4 What is the total no. of bids placed on the team that has won highest no. of matches?
#bid_team name changed to team_id for question 2 

select i.TEAM_ID 'Team ID', team_name 'Team Name', count(*) 'Total Bids'
from ipl_bidding_details i
join ipl_team k
on i.team_id = k.TEAM_ID 
where bid_status != 'cancelled' group by k.team_id
having k.team_id = (select team_id from ipl_team_standings order by matches_won desc limit 1);
...
((select total_points from ipl_team_standings s where s.team_id = t.team_id and tournmt_id = 2018) - 
(select total_points from ipl_team_standings s where s.team_id = t.team_id and tournmt_id = 2017) ) /
(select total_points from ipl_team_standings s where s.team_id = t.team_id and tournmt_id = 2017) * 100
as 'Jump from last year (%)'
